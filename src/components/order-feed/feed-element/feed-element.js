...
const FeedElement = ({order}) => {
  ...
  const visibaleIngredients = orderIngredients.slice(0, MAXIMUM_NUMBER_OF_RENDERED_INGREDIENTS);

  const hiddenIngredients = order.ingredients.length - MAXIMUM_NUMBER_OF_RENDERED_INGREDIENTS;
  ...
  return(
    <li className={styles['feed-container']} >
      ...
      <h3 className={`${styles['order-name']} text text_type_main-medium`} >{order.name}</h3>
      <div className={styles['order-details']}>
        <ul className={styles['order-ingredients']}>
          {visibaleIngredients.map((el, index) => {
            ingredientNumber -= 1;
            return (
              <li className={styles['order-ingredient']} key={index} style={{zIndex: ingredientNumber}}>
                <img src={el.image_mobile} alt='ingredient' />
              </li>
            )
          })}
            {
              order.ingredients.length > 6 ? (<div className={styles['order-ingredient-hidden']}>
                <p>{`+${hiddenIngredients}`}</p>
              </div>) : null
            }
        </ul>
        <div className={`${styles['order-price']} text text_type_digits-default`}>{orderPrice} <CurrencyIcon type={"primary"} /></div>
      </div>
    </li>
  );
}
